...
class SDTrainer(BaseSDTrainProcess):
    ...
    def backward_loss(self, loss: torch.Tensor):
        loss_scale = float(getattr(self, "_gradient_accumulation_loss_scale", 1.0))
        if loss_scale != 1.0:
    ...
    def _microbatch_effective_items(self, batch) -> int:
        if isinstance(batch, DataLoaderBatchDTO) and batch.file_items is not None:
            return max(1, len(batch.file_items))
    ...
    def cache_sample_prompts(self):
        ...
        if self.sample_config is not None and self.sample_config.samples is not None and len(self.sample_config.samples) > 0:
            ...
            for i in range(len(self.sample_config.prompts)):
                ...
                control_image_paths = gen_img_config.get_control_image_paths()
            ...
            with (network):
                ...
                if batch.unconditional_latents is not None or self.do_guided_loss:
                    ...
                    if self.train_config.diff_output_preservation or self.train_config.blank_prompt_preservation:
                        ...
                        self.backward_loss(preservation_loss)
    ...
    def hook_train_loop(self, batch: Union[DataLoaderBatchDTO, List[DataLoaderBatchDTO]]):
        ...
        previous_loss_scale = getattr(self, "_gradient_accumulation_loss_scale", 1.0)
        effective_items = [self._microbatch_effective_items(batch) for batch in batch_list]
        total_effective_items = max(1, sum(effective_items))
        try:
            for batch_index, batch in enumerate(batch_list):
                self._gradient_accumulation_loss_scale = (
                    effective_items[batch_index] / total_effective_items
                )
            ...
            self._gradient_accumulation_loss_scale = previous_loss_scale
