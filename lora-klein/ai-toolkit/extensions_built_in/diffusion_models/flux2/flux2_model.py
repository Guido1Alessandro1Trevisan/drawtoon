    ...
    def get_noise_prediction(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            ...
            def align_list_to_model_batch(items, name: str):
                if items is None:
                    return None
                items = list(items)
                if len(items) == batch_size:
                    aligned = items
                elif len(items) > 0 and batch_size % len(items) == 0:
                    repeat_count = batch_size // len(items)
                    aligned = []
                    for _ in range(repeat_count):
                        aligned.extend(items)
                else:
                    raise ValueError(
                        f"{name} length does not match model batch size: "
                        f"len={len(items)} batch_size={batch_size}"
                    )
                return aligned
            ...
            if batch is not None and batch_control_tensor_list is None and batch.control_tensor is not None:
                batch_control_tensor_list = []
                for b in range(int(batch.control_tensor.shape[0])):
                    batch_control_tensor_list.append(batch.control_tensor[b : b + 1])
            batch_control_tensor_list = align_list_to_model_batch(
                batch_control_tensor_list,
                "control_tensor_list",
            )
