...
class BaseModel:
    ...
    def _get_module_device(self, module):
        ...
        device = getattr(raw_module, "device", None)
        if device is not None:
            return torch.device(device)

        param = next(raw_module.parameters(recurse=True), None)
        if param is not None:
            return param.device

        buffer = next(raw_module.buffers(recurse=True), None)
    ...
    def _get_module_dtype(self, module):
        ...
        dtype = getattr(raw_module, "dtype", None)
    ...
    def supports_generate_image_batching(self) -> bool:
        return False

    def build_generate_image_batches(
        self,
        indexed_image_configs: List[tuple[int, GenerateImageConfig]],
        requested_batch_size: int,
    ) -> List[List[tuple[int, GenerateImageConfig]]]:
        return [[item] for item in indexed_image_configs]

    def generate_image_batch(
        self,
        pipeline,
        network,
        batch_items: List[tuple[int, GenerateImageConfig]],
    ) -> List[Image.Image]:
        raise NotImplementedError(
            "generate_image_batch must be implemented in child classes that support batched generation"
        )

    def _generate_one_image_for_config(
        self,
        pipeline,
        network,
        sample_index: int,
        ...
        if self.sample_prompts_cache is not None:
            conditional_embeds = self.sample_prompts_cache[sample_index]["conditional"].to(
                self.device_torch,
                dtype=self.torch_dtype,
            )
            unconditional_embeds = self.sample_prompts_cache[sample_index]["unconditional"].to(
                self.device_torch,
                dtype=self.torch_dtype,
            )
        else:
            ctrl_img = None
            control_image_paths = (
                gen_config.get_control_image_paths()
                if hasattr(gen_config, "get_control_image_paths")
                else []
            )
            has_control_images = len(control_image_paths) > 0
            if has_control_images and self.encode_control_in_text_embeddings:
                ctrl_img_list = []
                for control_image_path in control_image_paths:
                    control_image = Image.open(control_image_path).convert("RGB")
                    control_tensor = (
                        TF.to_tensor(control_image)
                        .unsqueeze(0)
                        .to(self.device_torch, dtype=self.torch_dtype)
                    )
                    ctrl_img_list.append(control_tensor)
        ...
        unet_dtype = self._get_module_dtype(self.unet)
    ...
    def _generate_images_in_batches(
        self,
        pipeline,
        network,
        ...
        indexed_image_configs = list(enumerate(image_configs))
        image_batches = self.build_generate_image_batches(
            indexed_image_configs,
            requested_batch_size,
        )

        total_images = len(image_configs)
        with tqdm(total=total_images, desc="Generating Images", leave=False) as progress_bar:
            for batch_items in image_batches:
                if len(batch_items) <= 1:
                    sample_index, gen_config = batch_items[0]
                    img = self._generate_one_image_for_config(
                        pipeline,
                        network,
                        sample_index,
                        gen_config,
                    )
                    gen_config.save_image(img, sample_index)
                    gen_config.log_image(img, sample_index)
                    self._after_sample_image(sample_index, total_images)
                    progress_bar.update(1)
                    flush()
                    continue

                images = self.generate_image_batch(
                    pipeline,
                    network,
                    batch_items,
                )
                if len(images) != len(batch_items):
                    raise ValueError(
                        "Batched validation generation returned the wrong number of images"
                    )
                for img, (sample_index, gen_config) in zip(images, batch_items):
                    gen_config.save_image(img, sample_index)
                    gen_config.log_image(img, sample_index)
                    self._after_sample_image(sample_index, total_images)
                    progress_bar.update(1)
    ...
    def generate_images(
        ...
        with network:
            with torch.no_grad():
                ...
                if batch_size > 1 and self.supports_generate_image_batching():
                    self._generate_images_in_batches(
                        pipeline,
                        network,
                        image_configs,
                        batch_size,
    ...
    def get_latent_noise(
        ...
        noise = torch.randn(
            ...
            device=self._get_module_device(self.unet),
        )
    ...
    def predict_noise(
        ...
        unet_device = self._get_module_device(self.unet)
        if unet_device != self.device_torch:
            try:
                self.unet.to(self.device_torch)
            except Exception:
                pass
        unet_dtype = self._get_module_dtype(self.unet)
        if unet_dtype != self.torch_dtype:
            self.unet.to(dtype=self.torch_dtype)
    ...
    def prepare_optimizer_params(
        ...
        if unet:
            ...
            trainable_scalars = sum(param.numel() for param in params if param.requires_grad)
            total_scalars = sum(param.numel() for param in self.unet.parameters())
            if get_accelerator().is_main_process:
                print_acc(
                    f"Found {trainable_tensors:,} trainable parameter tensors in unet "
                    f"({trainable_scalars:,} trainable / {total_scalars:,} total scalar parameters)"
                )
        ...
        if text_encoder:
            ...
            if get_accelerator().is_main_process:
                print_acc(
                    f"Found {len(params):,} trainable parameter tensors in text encoder"
                )
        ...
        if refiner:
            ...
            if get_accelerator().is_main_process:
                print_acc(f"Found {len(params):,} trainable parameter tensors in refiner")
    ...
    def save_device_state(self):
        ...
        if self.adapter is not None:
            if isinstance(self.adapter, IPAdapter):
                ...
                adapter_device = self._get_module_device(self.unet)
