...
def get_dataloader_from_datasets(
    ...
    for config in dataset_config_list:
        ...
        elif config.type == 'manifest':
            dataset = ManifestDataset(config, batch_size=batch_size, sd=sd)
            datasets.append(dataset)
            if getattr(dataset, 'use_bucket_batches', False):
                has_buckets = True
            if config.cache_latents or config.cache_latents_to_disk:
                is_caching_latents = True
        else:
    ...
    if has_buckets:
        ...
        if accelerator.num_processes > 1:
            # Bucketed datasets already emit full per-bucket batches as dataset items.
            # Shard those batch-items directly across ranks instead of letting
            # accelerate rebuild a batch sampler for a batch_size=None dataloader.
        ...
        data_loader._aitk_bucket_sharded = distributed_sampler is not None
    else:
        data_loader = DataLoader(
            concatenated_dataset,
...
def trigger_dataloader_setup_epoch(dataloader: DataLoader):
    dataloader = _unwrap_dataloader(dataloader)
    ...
    sampler_epoch = getattr(dataloader, "_aitk_sampler_epoch", 0)
    sampler = getattr(dataloader, "sampler", None)
    batch_sampler = getattr(dataloader, "batch_sampler", None)
    if hasattr(sampler, "set_epoch"):
        sampler.set_epoch(sampler_epoch)
        dataloader._aitk_sampler_epoch = sampler_epoch + 1
    elif hasattr(batch_sampler, "set_epoch"):
        batch_sampler.set_epoch(sampler_epoch)
        dataloader._aitk_sampler_epoch = sampler_epoch + 1
    elif hasattr(batch_sampler, "sampler") and hasattr(batch_sampler.sampler, "set_epoch"):
        batch_sampler.sampler.set_epoch(sampler_epoch)
        dataloader._aitk_sampler_epoch = sampler_epoch + 1
...
def _unwrap_dataloader(dataloader: DataLoader):
    return (
        getattr(dataloader, 'base_dataloader', None)
        or getattr(dataloader, 'dataloader', None)
        or dataloader
    )
