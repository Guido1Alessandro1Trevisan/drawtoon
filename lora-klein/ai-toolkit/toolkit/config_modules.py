...
def parse_gradient_accumulation_steps(raw, default: int = 1) -> int:
    if raw is None:
        raw = default
...
def get_ordered_control_image_paths(obj) -> List[str]:
    paths: List[str] = []
    first = getattr(obj, "ctrl_img_1", None) or getattr(obj, "ctrl_img", None)
    if first:
        paths.append(first)
    for idx in range(2, MAX_CONTROL_IMAGE_SLOTS + 1):
        path = getattr(obj, f"ctrl_img_{idx}", None)
        if path:
            paths.append(path)
    return paths


def build_control_image_kwargs(control_images: List[str]) -> Dict[str, str]:
    kwargs: Dict[str, str] = {}
    if not control_images:
        return kwargs
    if len(control_images) > MAX_CONTROL_IMAGE_SLOTS:
        raise ValueError(
            f"Received {len(control_images)} control images, but this fork supports "
            f"{MAX_CONTROL_IMAGE_SLOTS} unique controls."
        )
    kwargs["ctrl_img_1"] = control_images[0]
    for idx, path in enumerate(control_images[1:], start=2):
        if idx > MAX_CONTROL_IMAGE_SLOTS:
            break
        kwargs[f"ctrl_img_{idx}"] = path
    return kwargs
...
class LoggingConfig:
    def __init__(self, **kwargs):
        ...
        self.log_step_timing: bool = kwargs.get('log_step_timing', False)
...
class SampleItem:
    def __init__(
        ...
        **kwargs
        ...
        self.ctrl_img_4: Optional[str] = kwargs.get('ctrl_img_4', None)
        self.ctrl_img_5: Optional[str] = kwargs.get('ctrl_img_5', None)
        self.ctrl_img_6: Optional[str] = kwargs.get('ctrl_img_6', None)
        self.ctrl_img_7: Optional[str] = kwargs.get('ctrl_img_7', None)
        self.validation_sample_id: Optional[str] = kwargs.get('validation_sample_id', None)
        self.validation_target_path: Optional[str] = kwargs.get('validation_target_path', None)
        self.validation_caption: Optional[str] = kwargs.get('validation_caption', self.prompt)
        self.validation_control_paths: List[str] = kwargs.get(
    ...
    def get_control_image_paths(self) -> List[str]:
        return get_ordered_control_image_paths(self)

    def get_control_image_kwargs(self) -> Dict[str, str]:
        return build_control_image_kwargs(self.get_control_image_paths())
    ...
    def __init__(self, **kwargs):
        ...
        self.batch_size: int = max(
            ...
            int(
                kwargs.get(
                    'batch_size',
                    kwargs.get('validation_batch_size', 1),
                )
            ),
        )
        self.distributed_sampling: bool = kwargs.get(
            'distributed_sampling',
            kwargs.get('distributed_validation', False),
        )
        self.distributed_sample_total: Optional[int] = kwargs.get(
            'distributed_sample_total',
            kwargs.get('distributed_validation_total', None),
        )
        self.distributed_samples_per_rank: Optional[int] = kwargs.get(
            'distributed_samples_per_rank',
            kwargs.get('distributed_validations_per_rank', None),
        )
        self.distributed_sample_strategy: str = kwargs.get(
            'distributed_sample_strategy',
            'round_robin',
        )
        self.dynamic_validation_enabled: bool = kwargs.get(
            'dynamic_validation_enabled',
            False,
        )
        self.dynamic_validation_manifest_paths: List[str] = kwargs.get(
            'dynamic_validation_manifest_paths',
            [],
        )
        self.dynamic_validation_bucket_tolerance: int = kwargs.get(
            'dynamic_validation_bucket_tolerance',
            kwargs.get('bucket_tolerance', 16),
        )
        self.dynamic_validation_character_panel_count: Optional[int] = kwargs.get(
            'dynamic_validation_character_panel_count',
            None,
        )
        self.dynamic_validation_character_panel_fixed_count: Optional[int] = kwargs.get(
            'dynamic_validation_character_panel_fixed_count',
            None,
        )
...
class TrainConfig:
    def __init__(self, **kwargs):
        ...
        if 'gradient_accumulation' in kwargs:
            raise ValueError("Use train.gradient_accumulation_steps; train.gradient_accumulation is not supported")
        self.gradient_accumulation_steps = parse_gradient_accumulation_steps(
        ...
        if self.timestep_type in {'two_step', 'four_step', 'eight_step'}:
            raise ValueError(
                f"Unsupported timestep_type '{self.timestep_type}' in this fork. "
                "Supported values include sigmoid, linear, lognorm_blend, next_sample, weighted, and one_step."
            )
...
class DatasetConfig:
    ...
    def __init__(self, **kwargs):
        ...
        self.manifest_path: Optional[str] = kwargs.get('manifest_path', None)
        self.dataset_s3_path: Optional[str] = kwargs.get('dataset_s3_path', None)
        self.local_cache_dir: Optional[str] = kwargs.get('local_cache_dir', None)
        self.max_character_refs: int = kwargs.get('max_character_refs', 6)
        ...
        control_path_4 = kwargs.get('control_path_4', None)

        if any([control_path_1, control_path_2, control_path_3, control_path_4]):
            control_paths = []
            if control_path_1:
            ...
            if control_path_4:
                control_paths.append(control_path_4)
...
class GenerateImageConfig:
    def __init__(
            ...
            ctrl_img_4: Optional[str] = None,  # fourth control image for multi control model
            ctrl_img_5: Optional[str] = None,  # fifth control image for multi control model
        ...
        if control_images is not None:
            control_kwargs = build_control_image_kwargs(control_images)
            ctrl_img_1 = control_kwargs.get("ctrl_img_1", ctrl_img_1)
            ctrl_img_2 = control_kwargs.get("ctrl_img_2", ctrl_img_2)
            ctrl_img_3 = control_kwargs.get("ctrl_img_3", ctrl_img_3)
            ctrl_img_4 = control_kwargs.get("ctrl_img_4", ctrl_img_4)
            ctrl_img_5 = control_kwargs.get("ctrl_img_5", ctrl_img_5)
            ctrl_img_6 = control_kwargs.get("ctrl_img_6", ctrl_img_6)
            ctrl_img_7 = control_kwargs.get("ctrl_img_7", ctrl_img_7)
        ...
        self.ctrl_img_4 = ctrl_img_4
        self.ctrl_img_5 = ctrl_img_5
        self.ctrl_img_6 = ctrl_img_6
        self.ctrl_img_7 = ctrl_img_7
    ...
    def save_prompt_file(self, count: int = 0, max_count=0):
        ...
        with open(self.get_prompt_path(count, max_count), 'w') as f:
            ...
            for idx, control_path in enumerate(self.get_control_image_paths(), start=1):
                prompt += f' --ctrl_img_{idx} ' + control_path
            if self.ctrl_idx:
                prompt += ' --ctrl_idx ' + str(self.ctrl_idx)
        ...
        if self.prompt is not None and len(self.prompt) > 0:
            ...
            if len(p_split) > 1:
                for split in p_split[1:]:
                    ...
                    elif flag == 'ctrl_img_4':
                        self.ctrl_img_4 = content
                    elif flag == 'ctrl_img_5':
                        self.ctrl_img_5 = content
                    elif flag == 'ctrl_img_6':
                        self.ctrl_img_6 = content
                    elif flag == 'ctrl_img_7':
                        self.ctrl_img_7 = content
                    elif flag == 'ctrl_idx':
