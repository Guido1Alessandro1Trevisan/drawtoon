...
class FileItemDTO(
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.canonical_image_id = kwargs.get("canonical_image_id", None)
        preset_width = kwargs.get("preset_width", None)
        preset_height = kwargs.get("preset_height", None)
        preset_file_signature = kwargs.get("preset_file_signature", None)
        if self.canonical_image_id is None:
            relative_path = None
            if dataset_root is not None:
                try:
                    relative_path = os.path.relpath(self.path, dataset_root)
                except ValueError:
                    relative_path = None
            if relative_path is None:
                relative_path = os.path.basename(self.path)
            relative_stem = os.path.splitext(relative_path)[0].replace("\\", "/")
            self.canonical_image_id = relative_stem
        if dataset_root is not None:
        ...
        if preset_width is not None and preset_height is not None:
            file_signature = str(
                preset_file_signature
                or f"manifest:{self.canonical_image_id or self.path}"
            )
            w = int(preset_width)
            h = int(preset_height)
