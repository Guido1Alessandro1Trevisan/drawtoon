...
def default_execution_name(prefix: str) -> str:
    ...
    return f"{(normalized or 'page-caption')[:55]}-{timestamp}"
...
def main() -> None:
    ...
    cmd = subparsers.add_parser("caption-pages")
    cmd.add_argument("--bucket", default=DEFAULT_BUCKET)
    cmd.add_argument("--source-prefix", default=DEFAULT_SOURCE_PREFIX)
    cmd.add_argument("--annotation-prefix", default=DEFAULT_ANNOTATION_PREFIX)
    ...
    cmd.add_argument(
        "--require-annotations",
        action="store_true",
        help="Only caption pages that have a matching MAGI v3 annotation (for panel_count).",
    )
    ...
    payload: dict[str, Any] = {
        ...
        "require_annotations": bool(args.require_annotations),
    }
    arn = outputs["PageCaptionStateMachineArn"]
    if args.dry_run:
    ...
    response = client.start_execution(
        stateMachineArn=arn,
        name=default_execution_name(args.job_name or args.output_run or "page-caption"),
