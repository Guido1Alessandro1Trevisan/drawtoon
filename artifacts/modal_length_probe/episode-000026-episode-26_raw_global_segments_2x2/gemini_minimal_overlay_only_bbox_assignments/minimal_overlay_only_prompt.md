# Gemini Minimal Overlay-Only Prompt

```text
You are auditing Magi character boxes on one manga/webtoon review sheet.

Input:
- The image is the clean page/sheet with only simple colored Magi boxes and bbox ids drawn on top.
...
- Judge the artwork inside each box; the overlay is only for locating ids.
