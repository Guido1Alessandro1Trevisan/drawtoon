...
WORKER = ROOT / "aws" / "src" / "rfco_catalog_worker.py"
FUNCTION_NAME = "download-scraper-rfco-catalog-worker"
STATE_MACHINE_NAME = "download-scraper-rfco-catalog-map"
LAMBDA_ROLE_NAME = "download-scraper-rfco-catalog-lambda-role"
SFN_ROLE_NAME = "download-scraper-rfco-catalog-sfn-role"
...
def now_id() -> str:
    return "rfco_catalog_" + dt.datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")


def build_zip() -> bytes:
    buffer = io.BytesIO()
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as archive:
        archive.write(WORKER, "rfco_catalog_worker.py")
...
def put_lambda_policy(iam, *, bucket: str) -> None:
    ...
    iam.put_role_policy(RoleName=LAMBDA_ROLE_NAME, PolicyName="download-scraper-rfco-catalog-lambda", PolicyDocument=json.dumps(policy))
...
def put_sfn_policy(iam, *, bucket: str, region: str, account_id: str) -> None:
    ...
    iam.put_role_policy(RoleName=SFN_ROLE_NAME, PolicyName="download-scraper-rfco-catalog-sfn", PolicyDocument=json.dumps(policy))
...
def ensure_lambda(client, *, role_arn: str, args: argparse.Namespace) -> str:
    ...
    try:
        ...
        response = client.update_function_configuration(
            ...
            Role=role_arn,
            Handler="rfco_catalog_worker.handler",
...
def state_machine_definition(lambda_arn: str) -> str:
    definition = {
        "Comment": "RFCO catalog discovery and issue-page manifest builder.",
        "StartAt": "CatalogRows",
        "States": {
            "CatalogRows": {
                "Type": "Map",
                "Label": "RfcoCatalogRows",
                "ItemReader": {
...
def main() -> int:
    ...
    parser.add_argument("--task-type", choices=["discover_issues", "discover_listing_page", "build_page_manifest"], required=True)
    parser.add_argument("--issue-shard-prefix", default="")
    parser.add_argument("--page-shard-prefix", default="")
    parser.add_argument("--authorization-ref", default="MARVEL-DC-DRAWTOON-RFCO-FULL-PRIVATE-INGEST-2026-05-18")
    parser.add_argument("--approval-period", default="2026-05-18 through 2026-06-18")
    parser.add_argument("--output-root", default="datasets/pages/single")
    ...
    manifest_key = args.manifest_key or f"{args.manifest_prefix.strip('/')}/{run_id}/{args.task_type}_input.jsonl"
    audit_prefix = args.audit_prefix or f"{args.manifest_prefix.strip('/')}/{run_id}/{args.task_type}_audit/"
    issue_shard_prefix = args.issue_shard_prefix or f"{args.manifest_prefix.strip('/')}/{run_id}/issue_shards/"
    page_shard_prefix = args.page_shard_prefix or f"{args.manifest_prefix.strip('/')}/{run_id}/page_shards/"
    ...
    state_machine_arn = ensure_state_machine(sfn, definition=state_machine_definition(lambda_arn), role_arn=sfn_role)
    ...
    config: dict[str, Any] = {
        ...
        "bucket": args.bucket,
        "proxy_mode": args.proxy_mode,
        "issue_shard_prefix": issue_shard_prefix,
        "page_shard_prefix": page_shard_prefix,
        "authorization_ref": args.authorization_ref,
        "approval_period": args.approval_period,
    ...
    print(f"issue_shards=s3://{args.bucket}/{issue_shard_prefix}", flush=True)
    print(f"page_shards=s3://{args.bucket}/{page_shard_prefix}", flush=True)
